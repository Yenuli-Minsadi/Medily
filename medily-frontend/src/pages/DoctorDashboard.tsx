        ...
        <div className="p-6">
          ...
          {step === "enter_id" && (
            <div className="space-y-5">
              ...
              {/* Previous sessions this session */}
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-xl py-3.5 font-bold text-sm flex items-center justify-center gap-2 hover:opacity-90 transition-all shadow-lg shadow-indigo-200"
              >
          ...
          {step === "active" && session && (
            <div className="space-y-5">
              <div className="bg-gray-50 rounded-2xl p-4 flex items-center gap-4">
                <img
                  src={`https://ui-avatars.com/api/?name=${encodeURIComponent(session.patientName)}&background=4f46e5&color=fff`}
                  alt=""
              ...
              <div className="text-center py-6">
                <div className={`font-black text-6xl tabular-nums tracking-tight ${timerColor} transition-colors`}>
                  {fmt(elapsed)}
                </div>
                <div className="text-gray-400 text-sm mt-2">
                  {elapsed < 1800 ? "Session running" : elapsed < 3600 ? "Extended session" : "⚠️ Long session — consider wrapping up"}
                </div>
                {/* Progress bar */}
                <div className="w-full bg-gray-100 rounded-full h-1.5 mt-4 max-w-xs mx-auto">
                  <div
                    className={`h-1.5 rounded-full transition-all ${elapsed < 1800 ? "bg-emerald-400" : elapsed < 3600 ? "bg-amber-400" : "bg-red-400"}`}
                    style={{ width: `${Math.min((elapsed / 7200) * 100, 100)}%` }}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all resize-none"
                />
              </div>

              <button
                onClick={handleEndConsult}
                className="w-full bg-gray-900 text-white rounded-xl py-3.5 font-bold text-sm flex items-center justify-center gap-2 hover:bg-gray-800 transition-all"
              >
            ...
            <div className="space-y-5">
              <div className="text-center pt-2">
                ...
                <p className="text-gray-500 text-sm mt-1">
                  <span className="font-semibold text-gray-700">{session.patientName}</span> · Duration: <span className="font-mono font-semibold">{fmt(elapsed)}</span>
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className="bg-indigo-600 text-white rounded-xl py-4 font-bold text-sm flex flex-col items-center gap-2 hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200"
                >
                ...
                <button
                  ...
                  className="bg-gray-100 text-gray-700 rounded-xl py-4 font-bold text-sm flex flex-col items-center gap-2 hover:bg-gray-200 transition-all"
                >
          ...
          {step === "prescribe" && session && (
            <div className="space-y-5 text-center">
              ...
              <div className="flex gap-3">
                <button onClick={handleNewConsult} className="flex-1 border border-gray-200 text-gray-600 rounded-xl py-3 font-bold text-sm hover:border-indigo-300 hover:text-indigo-600 transition-colors">
                  New Consult
                </button>
                <button
                  onClick={() => { onGoToPrescriptions(session.patientId, session.patientName); onClose(); }}
                  className="flex-1 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-xl py-3 font-bold text-sm hover:opacity-90 transition-all shadow-lg shadow-indigo-200"
                >
                  Open Rx Form →
                </button>
              </div>
          ...
          {step === "done" && session && (
            <div className="space-y-5 text-center">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm mt-1">
                  Consultation with <span className="font-semibold text-gray-700">{session.patientName}</span> recorded as <span className="font-semibold text-gray-700">Consultation Only</span>.
                </p>
              </div>
              <div className="bg-gray-50 rounded-2xl p-4 text-left space-y-1.5">
                <div className="flex justify-between text-sm"><span className="text-gray-500">Patient</span><span className="font-bold text-gray-900">{session.patientName}</span></div>
                <div className="flex justify-between text-sm"><span className="text-gray-500">Duration</span><span className="font-mono font-semibold text-gray-700">{fmt(elapsed)}</span></div>
                <div className="flex justify-between text-sm"><span className="text-gray-500">Prescription</span><span className="text-gray-500 font-semibold">Not issued</span></div>
              </div>
              <div className="flex gap-3">
                <button onClick={handleNewConsult} className="flex-1 bg-indigo-600 text-white rounded-xl py-3 font-bold text-sm hover:bg-indigo-700 transition-colors">
                  New Consultation
                </button>
                <button onClick={onClose} className="flex-1 border border-gray-200 text-gray-600 rounded-xl py-3 font-bold text-sm hover:border-gray-300 transition-colors">
                  Close
                </button>
              </div>
            </div>
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4">
        ...
        <p className="text-gray-500 text-sm">The prescription has been sent to {patientName}'s dashboard.</p>
        <button onClick={() => { setSubmitted(false); setPatientName(""); setPatientId(""); setDiagnosis(""); setNotes(""); setMeds([{ name:"",dosage:"",frequency:"",duration:"",qty:"" }]); }}
          className="bg-indigo-600 text-white px-6 py-2.5 rounded-xl font-bold text-sm hover:bg-indigo-700 transition-colors mt-4">
          Write Another
        </button>
      </div>
    );
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input value={patientId} onChange={e => setPatientId(e.target.value)} placeholder="PAT-XXXX-XXXX"
              className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-mono outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all" />
          </div>
        </div>
        <div>
          ...
          <div key={i} className="p-4 bg-gray-50 rounded-xl space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input value={med.name} onChange={e => updateMed(i,"name",e.target.value)} placeholder="e.g. Amoxicillin"
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-indigo-400 bg-white transition-all" />
              </div>
              <div>
              ...
              <div>
                ...
                <input value={med.frequency} onChange={e => updateMed(i,"frequency",e.target.value)} placeholder="e.g. Twice daily"
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-indigo-400 bg-white transition-all" />
              </div>
              <div>
                <label className="text-xs text-gray-400 block mb-1">Duration</label>
                <input value={med.duration} onChange={e => updateMed(i,"duration",e.target.value)} placeholder="e.g. 7 days"
                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-indigo-400 bg-white transition-all" />
              </div>
            </div>
            <div>
              <label className="text-xs text-gray-400 block mb-1">Quantity</label>
              <input value={med.qty} onChange={e => updateMed(i,"qty",e.target.value)} placeholder="e.g. 14 tablets"
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-indigo-400 bg-white transition-all" />
            </div>
          </div>
        ))}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        ...
        <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3} placeholder="Instructions, warnings, follow-up advice…"
          className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all resize-none" />
      </div>

      {/* Submit */}
      <div className="flex gap-3">
        <button className="flex-1 border border-gray-200 text-gray-600 rounded-xl py-3.5 font-bold text-sm hover:border-gray-300 transition-colors">
          Save Draft
        </button>
        <button
          onClick={() => { if (patientName && diagnosis && meds[0].name) setSubmitted(true); }}
          disabled={!patientName || !diagnosis || !meds[0].name}
          className="flex-1 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-xl py-3.5 font-bold text-sm hover:opacity-90 transition-all shadow-lg shadow-indigo-200 disabled:opacity-40 disabled:cursor-not-allowed"
        >
          Issue Prescription
        </button>
      </div>
    ...
    <div className="flex min-h-screen bg-slate-50 font-sans">
      {showConsult && (
        <ConsultationModal
          ...
          onGoToPrescriptions={handleGoToPrescriptions}
        />
      )}
      ...
      `}>
        ...
        <div className="px-2 py-3 border-t border-gray-50 flex-shrink-0 space-y-0.5">
          ...
          <button
            ...
            title={!sidebarExpanded ? "Start Consultation" : undefined}
